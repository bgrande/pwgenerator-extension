...
async function getSettings() {
    return new Promise((resolve, reject) => {
        chrome.storage.sync.get('settings', function (items) {
            resolve((undefined !== items.settings) ? Helper.mergeObject(DEFAULT_SETTINGS, JSON.parse(items.settings)) : DEFAULT_SETTINGS);
        });
    });
}
...
(async function () {
    ...
    if (settings.useBrowserPopup === false) {
        ...
        $(BASE_NAME_POPUP + 'reload-title').innerText = chrome.i18n.getMessage("reloadTitle");
        $(BASE_NAME_POPUP + 'disable-title').innerText = chrome.i18n.getMessage("disableTitle");
